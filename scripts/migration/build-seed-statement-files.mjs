/**
 * Writes one .sql file per statement (ordered) for Supabase MCP `execute_sql`
 * (one statement per call). Skips raw_wp_items (large JSON; optional via full seed script).
 *
 * Usage: node scripts/migration/build-seed-statement-files.mjs
 */
...
const OUT_DIR = path.join(PROJECT_ROOT, "migration/output/seed_statements");
...
async function main() {
  ...
  for (const stmt of statements) {
    ...
    await writeFile(path.join(OUT_DIR, name), `${stmt}\n`, "utf8");
  }

  console.log(`Wrote ${statements.length} statement file(s) to ${OUT_DIR}`);
}
